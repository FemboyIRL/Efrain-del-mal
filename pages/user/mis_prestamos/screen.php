...
if (isset($_GET['searchBar']) && $_GET['searchBar'] != '') {
    if (isset($_GET['searchBar']) && $_GET['searchBar'] != '') {
        ...
        $query = "SELECT 
                  ...
                  WHERE 
                    prestamos.id_persona = $1 AND 
                    (libros.autor LIKE '%$searchBar%' OR libros.titulo LIKE '%$searchBar%' OR libros.isbn LIKE '%$searchBar%')
                  ORDER BY 
                    prestamos.fecha_prestamo DESC";
    }
}elseif (isset($_GET['excludedCategories']) && $_GET['excludedCategories'] != '') {
    $category = $_GET['excludedCategories'];
    $query = "SELECT 
                prestamos.id_prestamo,
                prestamos.fecha_prestamo,
                prestamos.fecha_devolucion,
              ...
              JOIN
                libros ON prestamos.codigo_libro = libros.id_libro
              WHERE 
                prestamos.id_persona = $1 AND libros.categoria != $2
              ORDER BY 
                prestamos.fecha_prestamo DESC";
    $params[] = $category;
                    ...
                    <tbody>
                        ...
                        if (pg_num_rows($result) > 0) {
                            while ($row = pg_fetch_assoc($result)) {
                                ...
                                echo "<td class='col-1'>  
                                    <button type='button' class='btn btn-dark' data-bs-toggle='modal' data-bs-target='#staticBackdrop' data-libro='" . $row["titulo"] . "'>
